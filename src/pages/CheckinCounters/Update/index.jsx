...
import { updateCheckIn } from '../../../state-manager/features/checkin-counters/thunks';
import Button from '../../../components/Shared/Button';

//FORMULARIO PARA EDITAR CHECK IN

const ModalEdit = ({setOpen}) => {

  const { check: checkinObtain } = useSelector(state => state.checkin)

  const [StatusDisplay, setStatusDisplay] = useState(checkinObtain.StatusDisplay)

  const [checkin, setCheckin] = useState({
        CheckinNumber: checkinObtain?.CheckinNumber,
        idAirline: checkinObtain?.idAirline?.idAirline,
        FlightNumber: checkinObtain?.FlightNumber,
        idFlightDestination: checkinObtain?.idFlightDestination?.idFlightDestination,
        idStatusFlight: checkinObtain?.idStatusFlight?.idStatusFlight,
        
  });

  const {loading: loadingAirlinesCounters, airline} =useSelector(state => state.airline)
  const {loading: loadingFlightDestination, flightdestination} =useSelector(state => state.flightdestination)
  const {loading: loadingStatusFlight, statusflight} =useSelector(state => state.statusflight)
  ...
  const onSubmit = () => {
    dispatch(updateCheckIn({id: checkinObtain.idCheckIn, StatusDisplay, ...checkin}))
    setOpen(false);
  }
  ...
  useEffect(() => {
    ...
    setStatusDisplay(checkinObtain?.StatusDisplay)
  }, [checkinObtain])
      ...
      >
        <div className="bg-slate-900	 p-8 rounded shadow-lg relative w-full max-w-7xl max-h-full overflow-y-auto">
          <div className="bg-slate-900 p-8 rounded shadow-lg">
            ...
            {/* 
            <div className="mb-4">
              <Field label="Numero de Check In:">
                <Input
                  value={checkin.CheckinNumber}
                  name="CheckinNumber"
            ...
            */}
            ...
            <Field label="Destino del Vuelo:">
              <select
                value={checkin?.idFlightDestination}
